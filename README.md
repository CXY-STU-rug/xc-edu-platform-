...
- [x] **v2.3 限时秒杀课（2026-06）**：抢购接口零数据库写入——**Redis Lua** 原子完成防重复+库存校验+扣减，成功发 **MQ** 异步生成待支付订单（复用既有订单/支付链路），前端轮询结果；唯一索引消费幂等，下单失败自动回补名额；`@Idempotent` 注解挡前端连点；建表脚本见 `sql/v2.3-seckill.sql`
